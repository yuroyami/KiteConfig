...
*Written by Claude Fable 5 after a full fresh-eyes audit of kitessot (every source, test,
...
3. **Version/changelog discipline**: this batch ships as **1.7.0**. Update `CHANGELOG.md`
   (Added/Fixed/Changed) and `gradle.properties` (`kiteSsot.version=1.7.0`) once at the end,
...
- **Config cache works.** `onlyIf { ext.… }` capturing the extension (4 sites in
  `KiteSsotPlugin.kt`) serializes fine (managed type), and `autoDetectLocales` reading the
...
- **`KGP_ON_CLASSPATH` reflective guard** (`KiteSsotPlugin.kt:478`) and **the AGP-type
  isolation pattern** (`KmpAndroidLibraryWiring` being its own class so AGP types never
  enter `KiteSsotPlugin`'s method descriptors) are load-bearing. Any new AGP- or KGP-typed
...
**Fix** in [GenerateIoWorkerTask.kt](src/main/kotlin/io/github/yuroyami/kitessot/GenerateIoWorkerTask.kt):
...
two top-level `kiteSsotOffload` declarations in one source set → "Conflicting overloads"
...
**Accept:** new functional-test step: run `generateKiteSsotIoWorkerJs` with package A, then
...
[KiteSsotPluginFunctionalTest.kt](src/test/kotlin/io/github/yuroyami/kitessot/KiteSsotPluginFunctionalTest.kt)
...
2. `internal fun applicationBuildConfigSpans(text: String): List<IntRange>`: - ids of objects whose span contains `isa = PBXNativeTarget` **and**
...
3. Change `rewritePbxproj` ([PbxprojRewrite.kt](src/main/kotlin/io/github/yuroyami/kitessot/PbxprojRewrite.kt)):
...
anywhere:
- Android: gate is `ext.versionName.isPresent` ([KiteSsotPlugin.kt:408](src/main/kotlin/io/github/yuroyami/kitessot/KiteSsotPlugin.kt#L408)).
- iOS: task passes `versionCode` only when `versionName.isPresent`
  ([SyncIosConfigTask.kt:76-77](src/main/kotlin/io/github/yuroyami/kitessot/SyncIosConfigTask.kt#L76)), and
  `rewritePbxproj` only writes `CURRENT_PROJECT_VERSION` inside the `versionName != null`
  branch ([PbxprojRewrite.kt:46-51](src/main/kotlin/io/github/yuroyami/kitessot/PbxprojRewrite.kt#L46)).
...
- `wireAndroidApp`: split the gate: `if (propagateVersion) { versionName only when versionName.isPresent; versionCode from
...
`pod 'X', :path => '../X'` match ([SharedModuleRewrite.kt:24](src/main/kotlin/io/github/yuroyami/kitessot/SharedModuleRewrite.kt#L24)).
...
2. Extend [CleanupLegacyAppLogoArtifactsTask.kt](src/main/kotlin/io/github/yuroyami/kitessot/CleanupLegacyAppLogoArtifactsTask.kt)
...
with `backup = false` ([SyncIosLogoTask.kt:95](src/main/kotlin/io/github/yuroyami/kitessot/SyncIosLogoTask.kt#L95)): on first contact that file is user-owned (often a full multi-size catalog).
...
fixture): pre-existing `Contents.json` gets a `.kitessot.bak`; orphan PNG produces the
...
**Fix** in `build.gradle.kts`: keep compiling on 21 but target 17: ```kotlin
...
`javap -v build/classes/kotlin/main/io/github/yuroyami/kitessot/VersionCodeKt.class | grep major`)
...
`embedAndSignAppleFrameworkForXcode` ([KiteSsotPlugin.kt:376-380](src/main/kotlin/io/github/yuroyami/kitessot/KiteSsotPlugin.kt#L376)).
...
- Apply `looksLikeLocaleQualifier` in `autoDetectLocales` ([KiteSsotPlugin.kt:245](src/main/kotlin/io/github/yuroyami/kitessot/KiteSsotPlugin.kt#L245))
...
.configureEach { compilerOptions.jvmTarget.set(JvmTarget.fromTarget(jv.toString())) }`: lazy, covers android-kotlin and kotlin-jvm compilations in that module. Skip silently when
...
module shapes; leave a field unset in `kiteSsot { }` to keep the module's own value."
...
1. **Gate logo validation on the toggle** ([KiteSsotPlugin.kt:101-123](src/main/kotlin/io/github/yuroyami/kitessot/KiteSsotPlugin.kt#L101)):
...
3. **`check(...)` → `GradleException`** for root-only enforcement
   ([KiteSsotPlugin.kt:21](src/main/kotlin/io/github/yuroyami/kitessot/KiteSsotPlugin.kt#L21)).
4. **Fix the false comment in `writeAtomically`** ([RewriteSafety.kt:74-78](src/main/kotlin/io/github/yuroyami/kitessot/RewriteSafety.kt#L74))
   AND make it true: before staging, delete sibling files matching
   `"${target.name}.*${BACKUP_SUFFIX}.tmp"` (crash leftovers).
5. **Plist blank-line fix** ([PlistSanitize.kt:201-210](src/main/kotlin/io/github/yuroyami/kitessot/PlistSanitize.kt#L201)):
...
6. **Reject Kotlin hard keywords in `ioWorkerPackage`**: extend the validation at
   [KiteSsotPlugin.kt:214-220](src/main/kotlin/io/github/yuroyami/kitessot/KiteSsotPlugin.kt#L214) with a
...
7. **`kiteSsotVerify` Android coverage**: add lines for `android { }` SDK values,
   `javaVersion`, logo config (FG/BG/color set-ness), interop/web toggles. Keep it
   read-only.
8. **knownRegions indent preservation** ([PbxprojRewrite.kt:72-81](src/main/kotlin/io/github/yuroyami/kitessot/PbxprojRewrite.kt#L72)):
   capture the whitespace run after the original `(` and reuse it instead of hardcoded
   `\n\t\t\t\t`; fall back to tabs when absent.
9. **Generated worker hygiene** ([IoWorkerGen.kt](src/main/kotlin/io/github/yuroyami/kitessot/IoWorkerGen.kt)):
...
### T15: `KiteSsotBuildInfo` runtime codegen (highest value per line)

New opt-in block:
```kotlin
kiteSsot { buildInfo { enabled = true; packageName = "com.acme.app" } }  // default pkg: kitessot.generated
```
New `GenerateBuildInfoTask` (`@CacheableTask`, mirror `GenerateIoWorkerTask` incl. the T02
cleanup) emitting into `build/generated/kitessot/commonMain/kotlin/<pkg>/KiteSsotBuildInfo.kt`:
```kotlin
public object KiteSsotBuildInfo {
...
```
Escape string literals properly (reuse a small `internal fun kotlinStringLiteral(s: String)`: test `$`, `"`, `\`, newline). Wire onto the shared module's `commonMain` via the same
...
### T16: Aggregate tasks + `kiteSsotDoctor`

- Register `kiteSsotSync` (dependsOn sanitize, syncIosConfig, syncIosLogo, syncAndroidLogo),
  `kiteSsotSyncIos`, `kiteSsotSyncAndroid`: plain lifecycle tasks, group `kitessot`.
- New `KiteSsotDoctorTask` (read-only, modeled on `KiteSsotVerifyTask`) checking, each with
...
**Accept:** functional test running `kiteSsotDoctor` on the iOS fixture asserting a PASS and
...
- [ ] CHANGELOG 1.7.0 section complete; `kiteSsot.version=1.7.0`; README truth pass done.
- [ ] No new AGP/KGP types leaked into `KiteSsotPlugin` method descriptors (functional tests
