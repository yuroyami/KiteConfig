...
class ConsoleStyleTest {
    ...
    private val plain = KiteSsotConsole(colored = false)
    private val rich = KiteSsotConsole(colored = true)

    @Test
    fun `a plain console emits the text untouched`() {
        assertEquals("[FAIL] KMPS021", plain.paint("[FAIL] KMPS021", KiteSsotStyle.FAIL))
        assertFalse(plain.paint("x", KiteSsotStyle.HEADING).contains(''))
    }

    @Test
    fun `a rich console wraps the text and always closes the sequence`() {
        val painted = rich.paint("hello", KiteSsotStyle.FAIL)
    ...
    fun `color never splits the text, so a grep for a finding still matches`() {
        ...
        val line = rich.paint("  [FAIL] KMPS021 iOS pbxproj: broken", KiteSsotStyle.FAIL)
        assertTrue(line.contains("[FAIL] KMPS021"), line)
    }

    @Test
    fun `styles are distinguishable from one another`() {
        val seen = KiteSsotStyle.entries.map { rich.paint("x", it) }.toSet()
        assertEquals(KiteSsotStyle.entries.size, seen.size, seen.toString())
    }
