...
/**
 * Every `kiteSsot { }` example in the public KDoc is executed against a real
 * Gradle build.
 *
...
class KdocExampleCompilationTest {
    ...
    /** Pull every complete `kiteSsot { }` fence out of the main source's KDoc. */
                ...
                file.readLines().forEach { raw ->
                    ...
                    when {
                        ...
                        inside && line.startsWith("```") -> {
                            ...
                            if (code.trimStart().startsWith("kiteSsot {")) {
    ...
    @Test
    fun `every documented kiteSsot example configures a real build`(@TempDir projectDir: File) {
