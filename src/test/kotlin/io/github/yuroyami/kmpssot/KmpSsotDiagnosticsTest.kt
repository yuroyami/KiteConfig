...
class KmpSsotDiagnosticsTest {
    ...
    fun `malformed pbxproj is a stable fail-closed finding`() {
        ...
        val findings = KmpSsotDiagnosticEngine.evaluate(
            quietContext(syncIos = true, pbxprojFile = pbxproj),
        )

        val scope = findings.single { it.id == "KMPS021" }
        assertEquals(KmpSsotDiagnosticSeverity.ERROR, scope.severity)
    ...
    fun `filesystem read failure becomes a finding instead of escaping`() {
        ...
        assertEquals(KmpSsotDiagnosticSeverity.ERROR, manifest.severity)
    ...
    fun `diagnostics reject configured files outside the root project`() {
        ...
        val finding = KmpSsotDiagnosticEngine.evaluate(
            quietContext(
                propagateAppName = true,
                appName = "Demo",
        ...
        assertEquals(KmpSsotDiagnosticSeverity.ERROR, finding.severity)
    ...
    fun `Android logo diagnostics require the installed launcher resource references`() {
        ...
        fun finding(icon: String, roundIcon: String?): KmpSsotDiagnostic {
            manifest.writeText(
                """
                <manifest xmlns:android="http://schemas.android.com/apk/res/android">
            ...
            return KmpSsotDiagnosticEngine.evaluate(
        ...
        assertEquals(KmpSsotDiagnosticSeverity.ERROR, stale.severity)
        ...
        assertEquals(KmpSsotDiagnosticSeverity.WARNING, missingRound.severity)

        val aligned = finding("@mipmap/ic_launcher", "@mipmap/ic_launcher_round")
        assertEquals(KmpSsotDiagnosticSeverity.PASS, aligned.severity)
    }
    ...
    fun `plist diagnostic reports KEEP conflicts as intentionally preserved warnings`() {
        ...
        assertEquals(KmpSsotDiagnosticSeverity.WARNING, finding.severity)
    ...
    fun `pbx diagnostic reports configured value drift and passes after rewrite`() {
        ...
        val drift = KmpSsotDiagnosticEngine.evaluate(context).single { it.id == "KMPS021" }
        assertEquals(KmpSsotDiagnosticSeverity.ERROR, drift.severity)
        ...
        val aligned = KmpSsotDiagnosticEngine.evaluate(context).single { it.id == "KMPS021" }
        assertEquals(KmpSsotDiagnosticSeverity.PASS, aligned.severity)
    }
    ...
    fun `iOS icon diagnostics reject a missing manifest-owned output`() {
        ...
        val manifest = project.resolve("ios/App/.kmpssot/$identity/owned-files-v1")
        OwnedOutputSafety.replaceInstalledFiles(
    ...
    fun `Android selector diagnostic rejects implicit ambiguity only for active app scope`() {
        val active = KmpSsotDiagnosticEngine.evaluate(
            quietContext(propagateAppName = true, appName = "Demo").copy(
                detectedAndroidApplicationProjects = listOf(":phone", ":tablet"),
            ),
        ).single { it.id == "KMPS070" }

        assertEquals(KmpSsotDiagnosticSeverity.ERROR, active.severity)
        ...
        val inactive = KmpSsotDiagnosticEngine.evaluate(
            quietContext().copy(detectedAndroidApplicationProjects = listOf(":phone", ":tablet")),
        ).single { it.id == "KMPS070" }
        assertEquals(KmpSsotDiagnosticSeverity.SKIPPED, inactive.severity)
    }
    ...
    fun `universal AppIcon diagnostic requires a compatible deployment target`() {
        val missing = KmpSsotDiagnosticEngine.evaluate(
            quietContext(syncIos = true, propagateLogo = true),
        ).single { it.id == "KMPS023" }
        assertEquals(KmpSsotDiagnosticSeverity.ERROR, missing.severity)

        val compatible = KmpSsotDiagnosticEngine.evaluate(
            quietContext(syncIos = true, propagateLogo = true).copy(iosDeploymentTarget = "12.0"),
        ).single { it.id == "KMPS023" }
        assertEquals(KmpSsotDiagnosticSeverity.PASS, compatible.severity)
    }

    @Test
    fun `active unsupported AGP and KGP versions are errors only when relevant`() {
        val relevant = KmpSsotDiagnosticEngine.evaluate(
            quietContext().copy(
                agpRequired = true,
                agpOnClasspath = true,
                activeAgpVersion = "9.2.0",
                kgpRequired = true,
                kgpOnClasspath = true,
        ...
        assertEquals(KmpSsotDiagnosticSeverity.ERROR, agp.severity)
        assertEquals("9.2.0", agp.actual)
        val kgp = relevant.single { it.id == "KMPS062" }
        assertEquals(KmpSsotDiagnosticSeverity.ERROR, kgp.severity)
        assertEquals("2.5.0", kgp.actual)

        val irrelevant = KmpSsotDiagnosticEngine.evaluate(
            quietContext().copy(
                agpOnClasspath = true,
                activeAgpVersion = "9.2.0",
                kgpOnClasspath = true,
    ...
    fun `json report is deterministic and escapes source text`() {
        val finding = KmpSsotDiagnostic(
            id = "KMPS777",
            severity = KmpSsotDiagnosticSeverity.ERROR,
            title = "A \"quoted\" title",
            detail = "line one\nline two\\tail\u2028\u2029\uD800",
            remediation = "tab\there",
        )

        val first = KmpSsotDiagnosticReports.json(listOf(finding))
        val second = KmpSsotDiagnosticReports.json(listOf(finding))
    ...
    fun `console diagnostics bound and escape uncontrolled finding text`() {
        ...
        val console = renderDiagnosticConsole(
            "probe",
            listOf(KmpSsotDiagnostic("KMPS777", KmpSsotDiagnosticSeverity.ERROR, raw, raw, raw)),
        )
    ...
    fun `sarif contains stable rule ids and only actionable results`() {
        val report = KmpSsotDiagnosticReports.sarif(
            listOf(
                KmpSsotDiagnostic("KMPS001", KmpSsotDiagnosticSeverity.PASS, "Pass", "ok"),
                KmpSsotDiagnostic("KMPS002", KmpSsotDiagnosticSeverity.WARNING, "Warn", "careful"),
                KmpSsotDiagnostic("KMPS003", KmpSsotDiagnosticSeverity.ERROR, "Error", "broken"),
            ),
        )
    ...
    fun `sarif locations are project relative portable and URI encoded`() {
        ...
        val relative = KmpSsotDiagnosticReports.sarif(
            listOf(
                KmpSsotDiagnostic(
                    "KMPS777",
        ...
        val windows = KmpSsotDiagnosticReports.sarif(
            listOf(
                KmpSsotDiagnostic(
                    "KMPS778",
                    KmpSsotDiagnosticSeverity.ERROR,
        ...
        val hostile = KmpSsotDiagnosticReports.sarif(
            listOf(
                KmpSsotDiagnostic(
                    "KMPS779",
                    KmpSsotDiagnosticSeverity.ERROR,
