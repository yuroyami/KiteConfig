...
class IoWorkerGenTest {
    ...
    fun `exposes the suspend offload entry point`() {
        ...
        assertTrue(src.contains("suspend fun kiteConfigOffload(jobJs: String, payload: String): String"))
        assertTrue(src.contains("timeoutMillis: Long"))
        assertTrue(src.contains("KITE_CONFIG_DEFAULT_TIMEOUT_MILLIS = 30_000L"))
    }
    ...
    fun `carries the generated-do-not-edit banner`() {
        val src = generateIoWorkerSource("kiteconfig.generated")
        assertTrue(src.contains("GENERATED by kiteconfig"))
    ...
    fun `uses the protocol v2 request envelope`() {
        ...
        assertTrue(src.contains("message.id = KITE_CONFIG_REQUEST_ID"))
    ...
    fun `reply echoes the correlation id and the handler matches it`() {
        ...
        assertTrue(src.contains("data.id == KITE_CONFIG_REQUEST_ID"))
    }
