 ...
 * classloader as kiteconfig and exercising the documented root `apply false` setup.
 */
class KiteConfigPluginFunctionalTest {
    ...
    fun `over-depth Swift discovery aborts the whole iOS text transaction`() {
        ...
        assertFalse(File(projectDir, "$pbxprojPath.kiteconfig.bak").exists(), "backup was created before planning finished")
    }
    ...
    fun `iOS logo takeover creates an external backup and ownership manifest`() {
        ...
        val recoveryRoot = File(projectDir, ".kiteconfig/recovery/ios-appicon/$catalogIdentity")
        ...
        val ownership = File(
            projectDir,
            "iosApp/iosApp/.kiteconfig/$catalogIdentity/owned-files-v1",
        )
    ...
    fun `web worker is selected explicitly and generated source compiles for browser JS`() {
        ...
        write(
            ...
            """
            package fixture
            import com.acme.gen.kiteConfigOffload
            suspend fun workerProbe(): String =
                kiteConfigOffload("(value) => value", "ok", 1_000L)
            """.trimIndent(),
        )
        ...
        val generated = File(
            projectDir,
            "shared/build/generated/kiteconfig/jsMain/kotlin/com/acme/gen/KiteConfigIoWorker.kt",
        )
        assertTrue(generated.isFile, "generated worker file missing: ${generated.path}")
        assertTrue(generated.readText().contains("suspend fun kiteConfigOffload"), generated.readText())
        ...
        val moved = File(
            projectDir,
            "shared/build/generated/kiteconfig/jsMain/kotlin/com/acme/other/KiteConfigIoWorker.kt",
        )
    ...
    fun `BuildConfig has complete identity and generated source compiles`() {
        ...
        val generated = File(
            projectDir,
            "shared/build/generated/kiteconfig/commonMain/kotlin/com/acme/gen/AppConfig.kt",
        )
    ...
    fun `fields-only BuildConfig never realizes identity task providers`() {
        ...
        write(
            "build.gradle.kts",
            """
            import io.github.yuroyami.kiteconfig.GenerateBuildConfigTask
        ...
        val source = File(
            projectDir,
            "shared/build/generated/kiteconfig/commonMain/kotlin/com/acme/fields/PublicConfig.kt",
        ).readText()
    ...
    fun `Android logo owns installed outputs and compileSdk 33 emits monochrome resources`() {
        ...
        val ownership = File(projectDir, "androidApp/src/main/.kiteconfig/android-logo-owned-files-v1")
        assertTrue(ownership.isFile, "Android logo ownership manifest missing")
        assertTrue(
            File(projectDir, ".kiteconfig/recovery/android-logo/removal-provenance.tsv").isFile,
    ...
    fun `version remains validated when an explicit versionCode bypasses derivation`() {
        ...
        val failure = runAndFail("help")
        assertTrue(failure.output.contains("kiteConfig { version }"), failure.output)
    ...
    @Test
    fun `root KiteConfig model is frozen before subprojects can mutate it`() {
        write("settings.gradle.kts", "rootProject.name = \"fixture\"\ninclude(\":late\")")
        write(
            "build.gradle.kts",
            """
            plugins { id("io.github.yuroyami.kiteconfig") }
            kiteConfig { appName = "Root authority" }
            """.trimIndent(),
        )
        write(
            "late/build.gradle",
            "rootProject.extensions.getByName('kiteConfig').appName.set('Late override')\n",
        )
    ...
    fun `auto-resolved Android app directory cannot be replaced by a subproject`() {
        ...
        write(
            "late/build.gradle",
            """
            rootProject.extensions.getByName('kiteConfig').modules.androidAppDirectory.set(
        ...
        write(
            ...
            kiteConfig {
                ...
                ios {
                    rewrite {
                        cleanPlist = true
                        onConflict = io.github.yuroyami.kiteconfig.PlistConflictPolicy.KEEP
                    }
                }
            }
            """.trimIndent(),
    ...
    fun `plan expands operations paths targets and policies without mutation`() {
        ...
        assertTrue(result.output.contains(".gradle/kiteconfig/rewrite.lock"), result.output)
    ...
    @Test
    fun `-Pkiteconfig dryRun and backups override the DSL for one invocation`() {
        writePng("art/fg.png", 8)
        write("settings.gradle.kts", "rootProject.name = \"fixture\"")
        write(
            "build.gradle.kts",
        ...
        val previewed = run("kiteInternalAndroidLogo", "-Pkiteconfig.dryRun=true")
    ...
    fun `the logo dry-run previews the deletions it would perform, not only the writes`() {
        ...
        val preview = run("kiteInternalAndroidLogo", "-Pkiteconfig.dryRun=true")
    ...
    fun `a misspelled safety flag fails the build and mutates nothing`() {
        ...
        val failure = runAndFail("kiteInternalAndroidLogo", "-Pkiteconfig.dryRun=treu")
        assertTrue(failure.output.contains("kiteconfig.dryRun"), failure.output)
        ...
        val backupsFailure = runAndFail("kiteInternalAndroidLogo", "-Pkiteconfig.backups=treu")
        assertTrue(backupsFailure.output.contains("kiteconfig.backups"), backupsFailure.output)
        assertFalse(File(projectDir, "src/main/res").exists(), "no source may be written")
    }
     ...
     * KiteConfig value only lands if KiteConfig's own callback was registered first.
     * `StaleName` in the output means KiteConfig lost that race.
     */
    @Test
    fun `the KiteConfig replaces a package name the desktop module declared itself`() {
     ...
     * no root `apply false`, landing it in a sibling classloader kiteconfig cannot see.
     */
    @Test
