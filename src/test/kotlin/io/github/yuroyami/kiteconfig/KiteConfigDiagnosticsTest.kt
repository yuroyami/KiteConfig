...
class KiteConfigDiagnosticsTest {
    ...
    fun `malformed pbxproj is a stable fail-closed finding`() {
        ...
        val findings = KiteConfigDiagnosticEngine.evaluate(
    ...
    fun `Android logo diagnostics require the installed launcher resource references`() {
        ...
        fun finding(icon: String, roundIcon: String?): KiteConfigDiagnostic {
            manifest.writeText(
                """
                <manifest xmlns:android="http://schemas.android.com/apk/res/android">
        ...
        assertEquals(KiteConfigDiagnosticSeverity.ERROR, stale.severity)
        ...
        assertEquals(KiteConfigDiagnosticSeverity.WARNING, missingRound.severity)
    ...
    fun `iOS icon diagnostics reject a missing manifest-owned output`() {
        ...
        val manifest = project.resolve("ios/App/.kiteconfig/$identity/owned-files-v1")
        OwnedOutputSafety.replaceInstalledFiles(
    ...
    fun `Android selector diagnostic rejects implicit ambiguity only for active app scope`() {
        val active = KiteConfigDiagnosticEngine.evaluate(
    ...
    fun `active unsupported AGP and KGP versions are errors only when relevant`() {
        val relevant = KiteConfigDiagnosticEngine.evaluate(
            quietContext().copy(
                agpRequired = true,
                agpOnClasspath = true,
        ...
        val irrelevant = KiteConfigDiagnosticEngine.evaluate(
            quietContext().copy(
                agpOnClasspath = true,
                activeAgpVersion = "9.4.0",
                kgpOnClasspath = true,
    ...
    fun `json report is deterministic and escapes source text`() {
        ...
        val first = KiteConfigDiagnosticReports.json(listOf(finding))
        val second = KiteConfigDiagnosticReports.json(listOf(finding))
