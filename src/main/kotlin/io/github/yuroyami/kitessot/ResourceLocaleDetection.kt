 ...
 * linked resource tree. Unsafe entries are ignored because explicit [locales][KiteSsotExtension.locales]
        ...
        for (entry in entries) {
            ...
            if (entryCount > maximumEntries) {
                throw GradleException(
                    "[kiteSsot] Refusing to inspect more than $maximumEntries Compose resource entries " +
                        "below ${diagnosticSafeText(composeResources.path)}; configure kiteSsot.locales explicitly.",
                )
            }
