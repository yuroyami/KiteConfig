...
internal object OwnedOutputSafety {
    internal const val OWNERSHIP_MANIFEST_NAME = ".kitessot-owned-files-v1"
    private const val OWNERSHIP_HEADER = "kitessot-owned-files-v1"
    private const val REMOVAL_HEADER = "kitessot-removal-provenance-v1"
    ...
    fun replaceGeneratedTree(
        ...
        if (root == boundary || !root.startsWith(boundary)) {
            throw GradleException(
                "[kiteSsot] Refusing generated output outside the plugin-owned directory: " +
            ...
            if (existing.isNotEmpty()) {
                ...
                try {
                    ...
                    recovery.targets.forEach { source ->
                        try {
                            ...
                            if (sha256(bytes) != expectedHash) {
                                throw GradleException("[kiteSsot] Installer recovery backup changed: $backup")
                            }
                            ...
                            if (
                                ...
                                throw GradleException("[kiteSsot] Restored installer takeover file failed verification: $source")
                            }
                    ...
                    if (restoreFailures.isNotEmpty()) {
                        ...
                        throw GradleException(
                            "[kiteSsot] Installer takeover failed and recovery of the original files was incomplete." +
                                preservedDetail,
                            failure,
                    ...
                    if (preservedExternalChanges.isNotEmpty()) {
                        throw GradleException(
                            "[kiteSsot] Installer takeover failed; rollback preserved newer external changes at: " +
                                preservedExternalChanges.joinToString() +
                    ...
                    throw GradleException(
                        "[kiteSsot] Installer takeover failed; original files were restored from the verified plan.",
                        failure,
                    )
                }
            } else {
    ...
    private fun prepareRemoval(
        ...
        if (backups.startsWith(root) || root.startsWith(backups)) {
            throw GradleException(
                "[kiteSsot] Logo migration backup directory must be separate from the Android resources: $backups",
            )
        }
        ...
        val targets = safeCandidates.mapNotNull { (_, path) ->
            ...
            if (!attrs.isRegularFile) {
                throw GradleException("[kiteSsot] Refusing to remove non-regular logo artifact: $path")
            }
            val size = attrs.size()
            if (size > MAX_MIGRATION_FILE_BYTES) {
                throw GradleException(
                    "[kiteSsot] Refusing to migrate unexpectedly large logo artifact ($size bytes): $path",
                )
            }
            ...
            if (snapshotBytes > MAX_TRANSACTION_SNAPSHOT_BYTES) {
                throw GradleException(
                    "[kiteSsot] Refusing a migration with more than " +
        ...
        targets.forEachIndexed { index, source ->
            ...
            if (Files.exists(destination, NOFOLLOW_LINKS)) {
                ...
                if (
                    ...
                    throw GradleException("[kiteSsot] Existing logo backup is invalid: $destination")
                }
            } else {
                ...
                if (sha256Bounded(destination, MAX_MIGRATION_FILE_BYTES, "logo migration backup") != hash) {
                    throw GradleException("[kiteSsot] Logo backup verification failed: $destination")
                }
            }
    ...
    internal fun deleteWithRollback(
        ...
        safeTargets.forEach { source ->
            ...
            if (sha256Bounded(backup, MAX_MIGRATION_FILE_BYTES, "verified migration backup") !=
                ...
                throw GradleException("[kiteSsot] Verified migration backup changed: $backup")
            }
        }
        ...
        try {
            ...
            if (recreated.isNotEmpty()) {
                throw GradleException(
                    "[kiteSsot] Logo artifacts were recreated during migration and were preserved: " +
            ...
            if (restoreFailures.isNotEmpty()) {
                throw GradleException(
                    "[kiteSsot] Logo migration failed and automatic rollback was incomplete. " +
            ...
            if (preservedExternalChanges.isNotEmpty()) {
                throw GradleException(
                    "[kiteSsot] Logo migration failed; rollback preserved newer external changes at: " +
                        preservedExternalChanges.joinToString() +
            ...
            throw GradleException(
                "[kiteSsot] Logo migration failed; all removed source artifacts were restored.",
                failure,
            )
        }
    }
    ...
    fun requireInputOutsideOutput(input: File, outputRoot: File, label: String) {
        ...
        if (source.startsWith(output)) {
            throw GradleException(
                "[kiteSsot] $label must be outside the generated output directory $output; got $source.",
            )
        }
    }
    ...
    fun requireInstallerInsideProject(directory: File, projectRoot: File, label: String) {
        ...
        if (destination == root || !destination.startsWith(root)) {
            throw GradleException(
                "[kiteSsot] Refusing $label outside the root project directory $root: $destination",
            )
        }
    }
    ...
    private fun requireProjectContainedPath(candidate: File, projectRoot: File, label: String): Pair<Path, Path> {
        ...
        if (Files.isSymbolicLink(lexicalRoot) || Files.isSymbolicLink(lexicalCandidate)) {
            throw GradleException("[kiteSsot] Refusing symlinked $label path: $lexicalCandidate")
        }
        if (lexicalCandidate.startsWith(lexicalRoot)) {
            ...
            lexicalRoot.relativize(lexicalCandidate).forEach { component ->
                ...
                if (Files.exists(cursor, NOFOLLOW_LINKS) && Files.isSymbolicLink(cursor)) {
                    throw GradleException("[kiteSsot] Refusing $label through symlink: $cursor")
                }
            }
        }
        ...
        if (resolved != root && !resolved.startsWith(root)) {
            throw GradleException("[kiteSsot] Refusing $label outside the root project directory $root: $resolved")
        }
    ...
    fun requireSafePath(file: File, label: String) {
        try {
            ...
            throw GradleException("[kiteSsot] Unsafe $label path: ${e.message}", e)
        }
    }
    ...
    private fun replaceOwnedFilesLocked(
        ...
        if (files.isEmpty()) throw GradleException("[kiteSsot] Refusing to install an empty owned output set.")
        if (files.size > MAX_OWNED_OUTPUT_ENTRIES) {
            throw GradleException(
                "[kiteSsot] Refusing to install more than $MAX_OWNED_OUTPUT_ENTRIES owned output files.",
            )
        }
        var desiredBytes = 0L
        files.forEach { (relative, bytes) ->
            if (bytes.size.toLong() > MAX_MIGRATION_FILE_BYTES) {
                throw GradleException(
                    "[kiteSsot] Refusing owned output larger than $MAX_MIGRATION_FILE_BYTES bytes: $relative",
                )
            }
            desiredBytes += bytes.size
            if (desiredBytes > MAX_TRANSACTION_SNAPSHOT_BYTES) {
                throw GradleException(
                    "[kiteSsot] Refusing owned outputs totaling more than " +
        ...
        if (desired.size != files.size) {
            throw GradleException("[kiteSsot] Duplicate normalized generated output paths are not allowed.")
        }
        ...
        previous.forEach { entry ->
            ...
            if (Files.exists(target, NOFOLLOW_LINKS)) {
                ...
                if (!attrs.isRegularFile) {
                    throw GradleException("[kiteSsot] Owned output is no longer a regular file: $target")
                }
                val actual = sha256Bounded(target, MAX_MIGRATION_FILE_BYTES, "owned output")
                if (actual != entry.sha256) {
                    throw GradleException(
                        "[kiteSsot] Refusing to replace modified generated output: $target. " +
        ...
        desired.keys.forEach { entry ->
            ...
            if (entry.relativePath !in previousByPath && Files.exists(target, NOFOLLOW_LINKS)) {
                throw GradleException(
                    "[kiteSsot] Refusing to overwrite unowned file at generated path: $target. " +
        ...
        if (exclusiveRoot && Files.exists(installationRoot, NOFOLLOW_LINKS)) {
            ...
            walkRegularFilesNoFollow(installationRoot).forEach { path ->
                ...
                if (relative != manifestInRoot && relative != lockInRoot && relative !in known) {
                    throw GradleException(
                        "[kiteSsot] Refusing to clean generated directory containing unowned file: $path. " +
        ...
        if (manifestBytes.size.toLong() > MAX_MANIFEST_BYTES) {
            throw GradleException(
                "[kiteSsot] Refusing an ownership manifest larger than $MAX_MANIFEST_BYTES bytes.",
            )
        }
    ...
    private fun commitOwnedFilesTransaction(
        ...
        paths.forEach { path ->
            ...
            val bytes = if (Files.exists(path, NOFOLLOW_LINKS)) {
                ...
                if (!attrs.isRegularFile || attrs.size() > MAX_MIGRATION_FILE_BYTES) {
                    throw GradleException("[kiteSsot] Output is too large or unsafe to transact: $path")
                }
            ...
            when {
                owned == null && bytes != null -> throw GradleException(
                    "[kiteSsot] Refusing to overwrite unowned file at generated path: $path. " +
                        "Move or back up that file, then retry.",
                )

                owned != null && bytes != null && sha256(bytes) != owned.sha256 -> throw GradleException(
                    "[kiteSsot] Refusing to replace modified generated output: $path. " +
                        "Move your changes to source control or delete the generated directory deliberately.",
                )
            }
            ...
            if (snapshotBytes > MAX_TRANSACTION_SNAPSHOT_BYTES) {
                throw GradleException(
                    "[kiteSsot] Refusing an output transaction with more than " +
                        "$MAX_TRANSACTION_SNAPSHOT_BYTES bytes of recovery state.",
                )
            }
            snapshots[path] = bytes
        ...
        try {
            ...
            if (rollbackFailures.isNotEmpty()) {
                throw GradleException(
                    "[kiteSsot] Owned-output installation failed and automatic rollback was incomplete.",
                    failure,
                ).also { combined -> rollbackFailures.forEach(combined::addSuppressed) }
            }
            if (preservedExternalChanges.isNotEmpty()) {
                throw GradleException(
                    "[kiteSsot] Owned-output installation failed; rollback preserved newer external changes at: " +
                        preservedExternalChanges.joinToString(),
                    failure,
                )
            }
            throw GradleException(
                "[kiteSsot] Owned-output installation failed; the previous output set was restored.",
                failure,
            )
        }
    }
    ...
    private fun verifySnapshotUnchanged(
        ...
        if (!unchanged) {
            throw GradleException("[kiteSsot] $label changed after its transaction snapshot: $path")
        }
    }
        ...
        try {
            if (snapshotBytes == null) {
                if (!moveNewWithoutReplacing(stagedReplacement, path)) {
                    throw GradleException("[kiteSsot] $label appeared before installation and was not overwritten: $path")
                }
                return
            }

            val parked = moveCurrentPathAside(path) ?: throw GradleException(
                "[kiteSsot] $label disappeared before installation and was not recreated: $path",
            )
            ...
            if (!currentStateEquals(parked, snapshotBytes, maximumBytes, label)) {
                if (!moveNewWithoutReplacing(parked, path)) {
                    ...
                    throw GradleException(
                        "[kiteSsot] $label changed during installation; newer entries were preserved at " +
                            "$path and $parked",
                    )
                }
                throw GradleException("[kiteSsot] $label changed during installation and was not overwritten: $path")
            }
            if (!moveNewWithoutReplacing(stagedReplacement, path)) {
                preserveParkedCurrent = true
                throw GradleException(
                    "[kiteSsot] $label was recreated during installation; the newer entry was preserved at " +
    ...
    private fun deleteSnapshotSafely(
        ...
        if (snapshotBytes == null) {
            if (Files.exists(path, NOFOLLOW_LINKS)) {
                throw GradleException("[kiteSsot] $label appeared before deletion and was preserved: $path")
            }
            return
        }

        val parkedCurrent = moveCurrentPathAside(path) ?: throw GradleException(
            "[kiteSsot] $label disappeared before deletion: $path",
        )
        var preserveParkedCurrent = false
        try {
            if (!currentStateEquals(parkedCurrent, snapshotBytes, maximumBytes, label)) {
                if (!moveNewWithoutReplacing(parkedCurrent, path)) {
                    preserveParkedCurrent = true
                    throw GradleException(
                        "[kiteSsot] $label changed during deletion; newer entries were preserved at " +
                            "$path and $parkedCurrent",
                    )
                }
                throw GradleException("[kiteSsot] $label changed during deletion and was preserved: $path")
            }
            Files.delete(parkedCurrent)
            if (Files.exists(path, NOFOLLOW_LINKS)) {
                throw GradleException("[kiteSsot] $label was recreated during deletion and was preserved: $path")
            }
        } finally {
            ...
            val unchanged = try {
                ...
                if (!moveNewWithoutReplacing(parkedCurrent, path)) {
                    ...
                    verificationFailure.addSuppressed(
                        GradleException(
                            "[kiteSsot] Could not restore unverifiable displaced content at $parkedCurrent; " +
    ...
    private fun restoreSnapshotIfStillTransactionState(
        ...
        val parent = path.parent ?: throw GradleException("[kiteSsot] Rollback target has no parent: $path")
        ...
        try {
            if (writtenBytes == null) {
                ...
                if (!moveNewWithoutReplacing(stagedSnapshot!!, path)) {
                    ...
                    throw GradleException(
                        "[kiteSsot] $label preserved a newer entry at $path; " +
            ...
            if (!currentStateEquals(parked, writtenBytes, maximumBytes, label)) {
                if (!moveNewWithoutReplacing(parked, path)) {
                    ...
                    throw GradleException(
                        "[kiteSsot] $label found newer displaced content at $parked and " +
    ...
    private fun stageRollbackSnapshot(path: Path, bytes: ByteArray): Path {
        val parent = path.parent ?: throw GradleException("[kiteSsot] Output has no parent: $path")
        requireSafeExistingComponents(parent)
        Files.createDirectories(parent)
        requireSafeExistingComponents(parent)
    ...
    private fun moveCurrentPathAside(path: Path): Path? {
        val parent = path.parent ?: throw GradleException("[kiteSsot] Transaction target has no parent: $path")
        requireSafeExistingComponents(parent)
        ...
        return try {
            ...
            if (!movedAttributes.isRegularFile || movedAttributes.isSymbolicLink || movedAttributes.isOther) {
                throw GradleException(
                    "[kiteSsot] Transaction target changed to a symlink or non-regular entry and was not read: $path",
                )
            }
            ...
            try {
                if (!moveNewWithoutReplacing(parked, path)) {
                    failure.addSuppressed(
                        GradleException(
                            "[kiteSsot] The displaced entry was preserved at $parked because another entry exists at $path.",
                        ),
                    )
                }
            } catch (restoreFailure: Exception) {
    ...
    private fun readOwnershipManifest(manifest: Path, expectedOwner: String): OwnershipSnapshot {
        ...
        if (!text.toByteArray(StandardCharsets.UTF_8).contentEquals(bytes)) {
            throw GradleException("[kiteSsot] Ownership manifest is not valid UTF-8: $manifest")
        }
        val lines = text.lineSequence().toList()
        if (lines.size < 2 || lines.first() != OWNERSHIP_HEADER) {
            throw GradleException("[kiteSsot] Unknown ownership manifest format: $manifest")
        }
        val ownerParts = lines[1].split('\t')
        if (ownerParts.size != 2 || ownerParts[0] != "owner" || ownerParts[1] != expectedOwner) {
            throw GradleException("[kiteSsot] Ownership manifest belongs to another generator: $manifest")
        }
        ...
        val inputFingerprint = if (fingerprintParts?.firstOrNull() == "input-fingerprint") {
            if (fingerprintParts.size != 2 || !fingerprintParts[1].matches(Regex("[0-9a-f]{64}"))) {
                throw GradleException("[kiteSsot] Malformed ownership input fingerprint: $manifest")
            }
            fingerprintParts[1]
        } else {
            null
        ...
        val entries = lines.drop(entryStart).filter(String::isNotBlank).map { line ->
            ...
            if (parts.size != 2 || !parts[0].matches(Regex("[0-9a-f]{64}"))) {
                throw GradleException("[kiteSsot] Malformed ownership manifest entry: $manifest")
            }
            val relative = validateRelativePath(parts[1])
            if (!seen.add(relative)) {
                throw GradleException("[kiteSsot] Duplicate ownership manifest entry for $relative")
            }
    ...
    private fun readRemovalManifest(manifest: Path): Pair<Set<String>, ByteArray?> {
        ...
        val text = String(bytes, StandardCharsets.UTF_8)
        if (!text.toByteArray(StandardCharsets.UTF_8).contentEquals(bytes)) {
            throw GradleException("[kiteSsot] Removal provenance manifest is not valid UTF-8: $manifest")
        }
        val lines = text.lineSequence().toList()
        if (lines.isEmpty() || lines.first() != REMOVAL_HEADER) {
            throw GradleException("[kiteSsot] Unknown removal provenance format: $manifest")
        }
        val records = lines.drop(1).filter(String::isNotBlank).onEach { line ->
            val parts = line.split('\t')
            if (parts.size != 3 || !parts[0].matches(Regex("[0-9a-f]{64}"))) {
                throw GradleException("[kiteSsot] Malformed removal provenance entry: $manifest")
            }
    ...
    internal fun walkRegularFilesNoFollow(
        ...
        fun record(path: Path) {
            ...
            if (entries > maximumEntries) {
                throw GradleException(
                    "[kiteSsot] Refusing to traverse more than $maximumEntries generated output entries: $path",
                )
            }
        }
            ...
            object : SimpleFileVisitor<Path>() {
                override fun preVisitDirectory(dir: Path, attrs: BasicFileAttributes): FileVisitResult {
                    ...
                    if (root.relativize(dir).nameCount > maximumDepth) {
                        throw GradleException(
                            "[kiteSsot] Refusing generated output deeper than $maximumDepth components: $dir",
                        )
                    }
                    if (attrs.isSymbolicLink || attrs.isOther) {
                        throw GradleException("[kiteSsot] Refusing to traverse unsafe generated directory: $dir")
                    }
                ...
                override fun visitFile(file: Path, attrs: BasicFileAttributes): FileVisitResult {
                    ...
                    if (attrs.isDirectory && root.relativize(file).nameCount > maximumDepth) {
                        throw GradleException(
                            "[kiteSsot] Refusing generated output deeper than $maximumDepth components: $file",
                        )
                    }
                    if (!attrs.isRegularFile) {
                        throw GradleException("[kiteSsot] Refusing special/symlink output entry: $file")
                    }
                    files.add(file)
                    return FileVisitResult.CONTINUE
                }
                ...
                override fun visitFileFailed(file: Path, exc: java.io.IOException): FileVisitResult {
                    record(file)
                    throw GradleException("[kiteSsot] Could not inspect generated output entry: $file", exc)
                }
            },
        )
        return files
    ...
    private fun validateRelativePath(value: String): String {
        if (value.isBlank() || '\\' in value || '\t' in value || '\n' in value || '\r' in value) {
            throw GradleException("[kiteSsot] Invalid portable relative output path: $value")
        }
    ...
    private fun requireLexicallyContainedNoSymlink(boundary: Path, candidate: Path, label: String) {
        ...
        if (path != root && !path.startsWith(root)) {
            throw GradleException("[kiteSsot] Refusing $label outside $root: $path")
        }
        if (Files.isSymbolicLink(root)) {
            throw GradleException("[kiteSsot] Refusing symlinked $label boundary: $root")
        }
        var cursor = root
        ...
        return try {
            Files.readAttributes(path, BasicFileAttributes::class.java, NOFOLLOW_LINKS).also {
                if (it.isSymbolicLink || it.isOther) {
                    throw GradleException("[kiteSsot] Refusing unsafe $label: $path")
                }
            }
        } catch (e: GradleException) {
            throw e
        } catch (e: Exception) {
            throw GradleException("[kiteSsot] Could not inspect $label: $path", e)
        }
    }
    ...
    private fun writeNewNoReplace(destination: Path, bytes: ByteArray) {
        ...
        Files.createDirectories(parent)
        requireSafeExistingComponents(parent)
        val temp = Files.createTempFile(parent, ".${destination.fileName}.kitessot-", ".tmp")
        try {
            Files.newByteChannel(temp, WRITE).use { channel ->
                var offset = 0
                while (offset < bytes.size) {
    ...
    private inline fun <T> withExclusiveLock(lock: Path, block: () -> T): T {
        val parent = lock.parent ?: throw GradleException("[kiteSsot] Lock has no parent: $lock")
        requireSafeExistingComponents(parent)
        Files.createDirectories(parent)
        requireSafeExistingComponents(parent)
        if (Files.exists(lock, NOFOLLOW_LINKS)) {
            val attrs = safeAttributes(lock, "ownership lock")
            if (!attrs.isRegularFile) throw GradleException("[kiteSsot] Invalid ownership lock: $lock")
        }
    ...
    private fun Path.normalizedSelectedEntry(label: String): Path {
        ...
        if (Files.isSymbolicLink(lexical)) {
            throw GradleException("[kiteSsot] Refusing symlinked $label: $lexical")
        }
        val parent = lexical.parent ?: throw GradleException("[kiteSsot] $label has no parent: $lexical")
    ...
    private inline fun consumeRegularFileBounded(
        ...
        if (!before.isRegularFile || before.size() > maximumBytes) {
            throw GradleException("[kiteSsot] $label is too large or unsafe: $path")
        }
        var total = 0L
        Files.newByteChannel(path, setOf(READ, NOFOLLOW_LINKS)).use { channel ->
            ...
            while (true) {
                ...
                if (count.toLong() > maximumBytes - total) {
                    throw GradleException("[kiteSsot] $label grew beyond $maximumBytes bytes while being read: $path")
                }
        ...
        if (
            ...
            throw GradleException("[kiteSsot] $label changed while being read: $path")
        }
    }
