...
abstract class GenerateDesktopSplashTask : DefaultTask() {
    ...
    /** Optional so that absent art fails with the kiteSsot message instead of Gradle's. */
    @get:InputFile
    @get:Optional
    ...
    fun generate() {
        val art = image.orNull?.asFile ?: throw GradleException(
            "[kiteSsot] The desktop splash has no art. Set kiteSsot { splash { image } } " +
                "or kiteSsot { logo { foreground } }.",
        )
        ...
        logger.info("[kiteSsot] Generated the desktop splash image: $target")
    }
    ...
    private fun plateColor(): Color = try {
        ...
        throw GradleException("[kiteSsot] The desktop splash background color is not usable. ${e.message}", e)
    }
