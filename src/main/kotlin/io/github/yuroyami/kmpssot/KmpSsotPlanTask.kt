...
abstract class KmpSsotPlanTask : DefaultTask() {

    init {
        group = "kmp-ssot"
        description = "Report selected kmp-ssot mutation paths and safety policies without changing files."
    ...
    fun report() {
        ...
        fun <T> resolved(label: String, fallback: T, read: () -> T): T = runCatching(read).getOrElse {
            logger.warn(
                "[kmpSsot] Plan could not resolve ${diagnosticSafeText(label)}: " +
        ...
        logger.lifecycle(
            buildString {
                appendLine("[kmpSsot] Mutation plan (read-only):")
