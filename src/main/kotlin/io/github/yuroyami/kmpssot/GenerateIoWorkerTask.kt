 ...
 *
 * Writes into `build/generated/kmpssot/<target>Main/kotlin` — a distinct,
 * plugin-owned dir, never the user's hand-authored source tree — keeping the
 * plugin's platform-tree-only contract intact. A checksum ownership manifest
...
abstract class GenerateIoWorkerTask : DefaultTask() {
    ...
    fun generate() {
        ...
        if (dryRun.getOrElse(false)) {
            logger.info("[kmpSsot] dryRun does not suppress build-owned Web Worker generation.")
        }
        val relative = "${pkg.replace('.', '/')}/KmpSsotIoWorker.kt"
        ...
        logger.info("[kmpSsot] Generated Web Worker helper: ${root.resolve(relative)}")
    }
}
