...
abstract class KmpSsotDiagnosticTaskBase : DefaultTask() {
    ...
    internal fun diagnosticFindings(): List<KmpSsotDiagnostic> {
        val resolutionFindings = mutableListOf<KmpSsotDiagnostic>()

        fun <T> resolve(id: String, name: String, fallback: T, value: () -> T): T =
            runCatching(value).getOrElse { failure ->
                resolutionFindings += KmpSsotDiagnostic(
                    id,
        ...
        val context = KmpSsotDiagnosticContext(
        ...
        val evaluated = runCatching { KmpSsotDiagnosticEngine.evaluate(context) }.getOrElse { failure ->
            listOf(
                KmpSsotDiagnostic(
                    "KMPS999",
                    KmpSsotDiagnosticSeverity.ERROR,
                    ...
                    "Report this as a kmp-ssot bug with --stacktrace output.",
                ),
            )
        }
 ...
 * contain errors; use [KmpSsotCheckTask] for CI enforcement.
 */
@DisableCachingByDefault(because = "Reads mutable source-tree state and always reports current diagnostics.")
abstract class KmpSsotDoctorTask : KmpSsotDiagnosticTaskBase() {

    init {
        description = "Diagnose kmp-ssot end-to-end without modifying files or failing on findings."
    }
...
internal fun renderDiagnosticConsole(heading: String, findings: List<KmpSsotDiagnostic>): String = buildString {
    appendLine("[kmpSsot] $heading:")
    findings.forEach { finding ->
        val status = when (finding.severity) {
            KmpSsotDiagnosticSeverity.PASS -> "PASS"
            KmpSsotDiagnosticSeverity.SKIPPED -> "SKIP"
            KmpSsotDiagnosticSeverity.WARNING -> "WARN"
            KmpSsotDiagnosticSeverity.ERROR -> "FAIL"
        }
