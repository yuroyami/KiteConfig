 ...
 * candidate is copied to a checksummed durable `.kmpssot/recovery` backup and recorded in a
...
abstract class CleanupLegacyAppLogoArtifactsTask : DefaultTask() {
    ...
    fun cleanup() {
        ...
        val ownershipManifest = resDir.parentFile.resolve(".kmpssot/android-logo-owned-files-v1")
        ...
        migrated.forEach { file ->
            ...
            if (dry) {
                logger.lifecycle("[kmpSsot][dry-run] would back up and remove logo artifact: $rel")
            } else {
                logger.lifecycle("[kmpSsot] Backed up and removed logo artifact: $rel")
            }
        }
        if (!dry && migrated.isNotEmpty()) {
            logger.lifecycle(
                "[kmpSsot] Logo migration recovery files and provenance: ${backupDir.asFile.get()}",
            )
        }
    }
}
