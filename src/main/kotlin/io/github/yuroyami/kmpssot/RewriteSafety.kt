...
/** Suffix appended to a user-owned file before kmpSsot rewrites it. */
...
internal fun applyTextRewritePlan(
    ...
    val lockDirectory = safeRoot.toPath().resolve(".gradle/kmpssot")
...
private fun stageSibling(target: Path, bytes: ByteArray): Path {
    ...
    val tmp = Files.createTempFile(target.parent, ".${target.fileName}.kmpssot-", ".tmp")
    try {
...
private fun moveCurrentPathAside(path: Path): Path? {
    ...
    val parked = Files.createTempFile(path.parent, ".${path.fileName}.kmpssot-current-", ".tmp")
    Files.delete(parked)
    return try {
...
internal fun renderUnifiedDiff(file: File, old: String, new: String): String {
    ...
    return buildString {
        ...
        appendLine("--- $safePath")
        appendLine("+++ $safePath (kmpSsot plan)")
