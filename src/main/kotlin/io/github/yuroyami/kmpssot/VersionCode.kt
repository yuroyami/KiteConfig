...
internal fun deriveVersionCode(versionName: String): Int {
    ...
    if (segments.size != 3) {
        throw GradleException(
            "kmpSsot: cannot derive a monotonic versionCode from versionName " +
                ...
                "x.y.z or set kmpSsot { versionCodeOverride = <int> }."
        )
    }
    val offending = segments.firstOrNull { !SEGMENT_RE.matches(it) }
    if (offending != null) {
        throw GradleException(
            "kmpSsot: cannot derive a versionCode from versionName " +
...
internal fun validateVersionCode(value: Int, property: String = "versionCodeOverride"): Int {
    if (value !in 1..MAX_ANDROID_VERSION_CODE) {
        throw GradleException(
            "kmpSsot { $property } must be in 1..$MAX_ANDROID_VERSION_CODE " +
...
internal fun validatePublishedVersionCode(next: Int?, published: Int): Int {
    ...
    val candidate = next ?: throw GradleException(
        "kmpSsot { android { publishedVersionCode } } requires versionName or versionCodeOverride.",
    )
    validateVersionCode(candidate)
    if (candidate <= published) {
        throw GradleException(
            "kmpSsot resolved Android versionCode $candidate must be greater than the published " +
