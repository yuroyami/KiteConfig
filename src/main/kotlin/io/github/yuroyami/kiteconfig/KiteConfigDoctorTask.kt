...
abstract class KiteConfigDiagnosticTaskBase : DefaultTask() {
    ...
    internal fun diagnosticFindings(): List<KiteConfigDiagnostic> {
        val resolutionFindings = mutableListOf<KiteConfigDiagnostic>()

        fun <T> resolve(id: String, name: String, fallback: T, value: () -> T): T =
            runCatching(value).getOrElse { failure ->
                resolutionFindings += KiteConfigDiagnostic(
                    id,
        ...
        val evaluated = runCatching { KiteConfigDiagnosticEngine.evaluate(context) }.getOrElse { failure ->
            listOf(
                KiteConfigDiagnostic(
                    ...
                    "Report this as a kiteconfig bug with --stacktrace output.",
                ),
            )
        }
 ...
 * contain errors; use [KiteConfigCheckTask] for CI enforcement.
 *
...
abstract class KiteConfigDoctorTask : KiteConfigDiagnosticTaskBase() {
    ...
    init {
        description = "Diagnose kiteconfig end-to-end without modifying files or failing on findings."
    }
...
internal fun renderDiagnosticConsole(
    heading: String,
    findings: List<KiteConfigDiagnostic>,
    console: KiteConfigConsole = KiteConfigConsole(colored = false),
): String = buildString {
    appendLine(console.paint("[kiteConfig] $heading", KiteConfigStyle.HEADING))
    findings.forEach { finding ->
        val (status, style) = when (finding.severity) {
            KiteConfigDiagnosticSeverity.PASS -> "PASS" to KiteConfigStyle.PASS
            KiteConfigDiagnosticSeverity.SKIPPED -> "SKIP" to KiteConfigStyle.SKIP
            KiteConfigDiagnosticSeverity.WARNING -> "WARN" to KiteConfigStyle.WARN
        ...
        finding.remediation?.let {
            appendLine(console.paint("         Fix: " + diagnosticSafeText(it), KiteConfigStyle.MUTED))
        }
    }
    ...
    val summaryStyle = when {
        errors > 0 -> KiteConfigStyle.FAIL
        warnings > 0 -> KiteConfigStyle.WARN
        else -> KiteConfigStyle.PASS
    }
