 ...
 * ```
 *
 * Opening this block turns generation on. KiteConfig writes one Kotlin object into
 ...
 *
 * @see KiteConfigModulesExtension.shared for selecting the module explicitly.
 * @see includeIdentity to generate fields only.
 */
abstract class KiteConfigBuildConfigExtension {
     ...
     *
     * KiteConfig resolves the provider when it generates the source. This can keep
    ...
    private fun missingFieldValue(name: String): String = throw GradleException(
        "kiteConfig { buildConfig { stringField(\"$name\", ...) } } was given a provider with no " +
    ...
    private fun String.checkedFieldName(): String = apply {
        require(isValidKotlinIdentifier(this)) {
            "kiteConfig { buildConfig } field name \"$this\" is not a valid Kotlin identifier."
        }
    }
}
