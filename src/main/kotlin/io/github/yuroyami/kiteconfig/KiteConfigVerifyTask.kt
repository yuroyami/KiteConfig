 ...
 * than aborting the report. Use [KiteConfigPlanTask] to inspect mutation policies
 * and [KiteConfigCheckTask] to gate CI on diagnostics.
 *
...
abstract class KiteConfigVerifyTask : DefaultTask() {
    ...
    fun report() {
        ...
        val console = KiteConfigConsole(colorEnabled.getOrElse(false))
        val root = runCatching { projectRootDir.asFile.orNull?.toPath() }.getOrNull()

        fun section(title: String) = console.paint("  $title", KiteConfigStyle.SECTION)
        fun rows(vararg entries: Pair<String, String>) =
            alignedRows(entries.toList(), indent = "    ", console = console)

        logger.lifecycle(
            buildString {
                appendLine(console.paint("[kiteConfig] Resolved single source of truth", KiteConfigStyle.HEADING))
    ...
    private fun presence(
        ...
        console: KiteConfigConsole,
    ): String {
        return runCatching {
            val f = p.asFile.orNull ?: return console.paint("[path unset]", KiteConfigStyle.MUTED)
            val path = f.toPath()
            val shown = root?.let { relativeDisplayPath(it, path) } ?: f.path
            val display = console.paint(diagnosticSafeText(shown), KiteConfigStyle.PATH)
            when {
                java.nio.file.Files.isSymbolicLink(path) ->
                    console.paint("SYMLINK", KiteConfigStyle.WARN) + " $display"
                java.nio.file.Files.isRegularFile(path, java.nio.file.LinkOption.NOFOLLOW_LINKS) ->
                    console.paint("found", KiteConfigStyle.PASS) + "   $display"
                java.nio.file.Files.exists(path, java.nio.file.LinkOption.NOFOLLOW_LINKS) ->
                    console.paint("OTHER", KiteConfigStyle.WARN) + "   $display"
                else -> console.paint("MISSING", KiteConfigStyle.FAIL) + " $display"
            }
        }.getOrElse { console.paint("ERROR   (${diagnosticExceptionSummary(it)})", KiteConfigStyle.FAIL) }
    }
