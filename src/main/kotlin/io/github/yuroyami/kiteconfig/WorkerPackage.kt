 ...
 * the generated `KiteConfigIoWorker.kt` fail to compile with a confusing error far from
...
internal fun invalidWorkerPackageReason(pkg: String): String? {
    ...
    if (keyword != null) {
        return "segment \"$keyword\" is a Kotlin hard keyword, so the generated file would not compile. " +
            "Rename that segment (e.g. \"${keyword}_\")."
    }
    if (pkg.split('.').any { it == "_" }) {
        return "contains the reserved underscore-only segment \"_\", so the generated file would not compile."
    }
    return null
}
