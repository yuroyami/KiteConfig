 ...
 * method descriptor. Were that on [KiteConfigPlugin] itself, Gradle would fail to *decorate*
 ...
 * hook that runs after the module's own DSL block but before variants lock, so the KiteConfig
...
internal object KmpAndroidLibraryWiring {
    ...
    fun apply(project: Project, ext: KiteConfigExtension, resilient: Boolean): Boolean {
            ...
            project.wireValueGroup(resilient, "the SDK levels") {
                if (ext.effectiveApplySdkLevels.get()) {
                    ...
                    if (sdk.targetSdk.isPresent || sdk.ndk.isPresent) {
                        project.logger.info(
                            "[kiteConfig] ${project.path}: targetSdk/ndkVersion ignored: the KMP Android library " +
                                "DSL exposes neither."
                        )
                    }
                }
