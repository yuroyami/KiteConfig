 ...
 * set, so it cannot read KiteConfig's internal Kotlin model directly. Resolving on
 ...
 */
internal fun KiteConfigExtension.resolveAgp8Inputs(project: org.gradle.api.Project, resilient: Boolean): Agp8AndroidInputs {
    fun <T> resolve(group: String, read: () -> T): T? = if (!resilient) {
        ...
        try {
            ...
            project.logger.info(
                "[kiteConfig] ${project.path}: $group not applied on this diagnostic invocation: ${failure.message}"
            )
            null
        }
    }
