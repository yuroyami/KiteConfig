...
abstract class KiteConfigPlanTask : DefaultTask() {
    ...
    init {
        group = "kiteconfig"
        description = "Report selected kiteconfig mutation paths and safety policies without changing files."
    ...
    fun report() {
        ...
        fun <T> resolved(label: String, fallback: T, read: () -> T): T = runCatching(read).getOrElse {
            logger.warn(
                "[kiteConfig] Plan could not resolve ${diagnosticSafeText(label)}: " +
        ...
        fun bullet(text: String, style: KiteConfigStyle = KiteConfigStyle.MUTED) =
            "    " + console.paint("- ", KiteConfigStyle.MUTED) + console.paint(text, style)

        logger.lifecycle(
            buildString {
                appendLine(console.paint("[kiteConfig] Mutation plan (read-only)", KiteConfigStyle.HEADING))
                appendLine(section("Operations"))
                values(resolvedOperations, "[none selected]").forEach {
                    appendLine(bullet(diagnosticSafeText(it), KiteConfigStyle.WARN))
                }
                ...
                values(resolvedPaths, "[none]").forEach { configured ->
                    appendLine(bullet(diagnosticSafeText(describePath(root, configured)), KiteConfigStyle.PATH))
                }
                ...
                values(resolvedChanges, "[not calculated]").forEach {
                    appendLine(bullet(diagnosticSafeText(it), KiteConfigStyle.MUTED))
                }
                if (resolvedNotes.isNotEmpty()) {
                    appendLine(section("Notes"))
                    resolvedNotes.forEach { appendLine(bullet(diagnosticSafeText(it))) }
                }
                append(console.paint("  No files were changed.", KiteConfigStyle.PASS))
            },
        )
    }
