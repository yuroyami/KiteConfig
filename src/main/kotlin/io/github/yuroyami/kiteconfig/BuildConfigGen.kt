    ...
    data class StringValue(override val name: String, val value: String) : BuildConfigField {
        init {
            require(value.length <= MAX_BUILD_CONFIG_STRING_CHARS) {
                "kiteConfig { buildConfig } String field \"$name\" exceeds " +
    ...
    data class DoubleValue(override val name: String, val value: Double) : BuildConfigField {
        init {
            require(value.isFinite()) {
                "kiteConfig { buildConfig } Double field \"$name\" must be finite; got $value."
            }
        }
...
internal fun generateBuildConfigSource(
    ...
    require(customFields.size <= MAX_BUILD_CONFIG_FIELDS) {
        "kiteConfig { buildConfig } supports at most $MAX_BUILD_CONFIG_FIELDS custom fields; " +
            "got ${customFields.size}."
    }
    var transportChars = 0L
    customFields.forEach { field ->
        require(field.length <= MAX_BUILD_CONFIG_FIELD_TRANSPORT_CHARS) {
            "kiteConfig { buildConfig } field transport exceeds " +
    ...
    require(duplicateNames.isEmpty()) {
        "kiteConfig { buildConfig } contains duplicate field name(s): ${duplicateNames.joinToString()}"
    }
    if (includeIdentity) {
        val collisions = parsedFields.map { it.name }.filter { it in IDENTITY_FIELD_NAMES }.sorted()
        require(collisions.isEmpty()) {
            "kiteConfig { buildConfig } custom field(s) collide with generated identity field(s): " +
    ...
    return """
        |/*
        | * GENERATED by kiteconfig (io.github.yuroyami.kiteconfig). DO NOT EDIT.
        | * Owned and regenerated when inputs change by `kiteInternalBuildConfig`.
        | */
        |package $packageName
        |
...
internal fun parseBuildConfigField(line: String): BuildConfigField {
    val match = FIELD_LINE_RE.matchEntire(line)
        ?: throw IllegalArgumentException(
            "Invalid kiteConfig buildConfig field \"$line\". Use stringField/intField/longField/" +
    ...
    require(isValidKotlinIdentifier(name)) {
        "kiteConfig { buildConfig } field name \"$name\" is not a valid Kotlin identifier."
    }
