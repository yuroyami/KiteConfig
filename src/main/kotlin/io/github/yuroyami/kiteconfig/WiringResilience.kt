 ...
 * diagnostic runs when those values live only in `kiteConfig { }`.
 */
internal fun Project.wireValueGroup(resilient: Boolean, group: String, write: () -> Unit) {
    ...
    try {
        ...
        logger.info("[kiteConfig] $path: $group not applied on this diagnostic invocation: ${failure.message}")
    }
}
 ...
 */
internal class ConfigDriftLog(private val project: Project) {
    ...
    fun report() {
        ...
        project.logger.warn(
            "[kiteConfig] ${project.path} declares values the single source of truth replaces: " +
                "${replaced.joinToString()}. Delete these module declarations; the kiteConfig { } block owns them."
        )
    }
}
