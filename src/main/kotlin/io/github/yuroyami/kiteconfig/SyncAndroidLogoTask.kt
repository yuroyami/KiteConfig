...
abstract class SyncAndroidLogoTask : DefaultTask() {
    ...
    fun sync() {
        ...
        if (fgFile == null) {
            logger.lifecycle("[kiteConfig] Android logo sync skipped: logo { foreground } is not configured.")
            return
        }
        if (!fgFile.exists()) {
            throw GradleException(
                "[kiteConfig] logo { foreground } points to a missing file: " +
        ...
        val bg: BufferedImage = if (backgroundColorHex.isPresent) {
            ...
            if (color.alpha < 255) {
                logger.warn("[kiteConfig] logo { backgroundColor } $hex is semi-transparent, so it will be flattened over white to match the iOS icon.")
            ...
            if (bgFile == null) {
                throw GradleException(
                    "[kiteConfig] Android logo requires logo { background } or logo { backgroundColor }.",
                )
            }
            if (!bgFile.exists()) {
                throw GradleException(
                    "[kiteConfig] logo { background } points to a missing file: " +
            ...
            if (decoded.image.width != decoded.image.height) {
                logger.warn("[kiteConfig] logo { background } is not square (${decoded.image.width}×${decoded.image.height}), so it will be center-cropped to fill the canvas.")
            }
        ...
        if (fg.width != fg.height) {
            logger.warn("[kiteConfig] logo { foreground } is not square (${fg.width}×${fg.height}), so it will be letterboxed inside the safe zone, preserving aspect ratio.")
        }
        if (!fg.colorModel.hasAlpha()) {
            logger.warn("[kiteConfig] logo { foreground } has no alpha channel, so it will fully cover the background. Use a PNG with transparency for proper layering.")
        }
        if (fg.width < 432) {
            logger.warn("[kiteConfig] logo { foreground } is ${fg.width}px wide. Use ≥432px (the xxxhdpi adaptive size) to avoid upscaling artifacts.")
        }
        ...
        if (dry) {
            rendered.keys.forEach { logger.lifecycle("[kiteConfig][dry-run] would write Android logo: $it") }
            val projectRoot = projectRootDir.asFile.get().toPath()
            takeoverFiles.filter(File::exists).forEach { file ->
                logger.lifecycle(
                    "[kiteConfig][dry-run] would take over Android logo (backed up, then removed): " +
        ...
        val manifest = resDir.parentFile.resolve(".kiteconfig/android-logo-owned-files-v1")
        ...
        logger.lifecycle("[kiteConfig] Android logo synced from ${fgFile.name} + $bgDescription.")
    }
    ...
    private fun failOnTemplateCollisions(resDir: File, collisions: List<File>) {
        ...
        throw GradleException(
            "[kiteConfig] Found ${collisions.size} template launcher icon(s) that collide with the " +
                ...
                Files.newDirectoryStream(directory).use { entries ->
                    for (path in entries) {
                        ...
                        if (entryCount > maximumEntries) {
                            throw GradleException(
                                "[kiteConfig] Refusing to scan more than $maximumEntries Android mipmap " +
                        ...
                        if (attrs.isSymbolicLink || attrs.isOther) {
                            throw GradleException(
                                "[kiteConfig] Refusing unsafe launcher resource entry: " +
