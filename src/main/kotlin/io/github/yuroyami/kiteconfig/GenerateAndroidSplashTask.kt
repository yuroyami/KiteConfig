...
private const val SPLASH_RESOURCE_FILE = "kiteconfig_splash.xml"
...
abstract class GenerateAndroidSplashTask : DefaultTask() {
    ...
    fun generate() {
        ...
        val artFile = image.asFile.orNull ?: throw GradleException(
            "[kiteConfig] The Android splash has no art. Set splash { image }, or logo { foreground } " +
        ...
        if (color == null) {
            logger.warn(
                "[kiteConfig] The Android splash has no plate color, so Android 12 keeps the window " +
        ...
        logger.info("[kiteConfig] Generated Android splash resources: ${outputDir.get().asFile}")
    }
    ...
    private fun validatedColor(value: String?, label: String): String? {
        ...
        try {
            ...
            throw GradleException(
                "[kiteConfig] $label must be #RRGGBB or #AARRGGBB (Android convention, alpha first), " +
    ...
    private fun validatedThemeName(): String {
        ...
        if (name.isEmpty()) {
            throw GradleException(
                "[kiteConfig] The Android splash needs the theme it inherits. Set " +
                    "splash { android { theme = \"AppTheme\" } } to the theme your manifest already names.",
            )
        }
        if (!ANDROID_STYLE_PARENT.matches(name)) {
            throw GradleException(
                "[kiteConfig] splash { android { theme } } \"${diagnosticSafeText(name, 120)}\" is not a style " +
    ...
    private fun decodeSplashArt(file: File, label: String): BufferedImage = try {
        ...
        throw GradleException("[kiteConfig] ${invalid.message}", invalid)
    }
}
