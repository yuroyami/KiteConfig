...
/** Suffix appended to a user-owned file before kiteConfig rewrites it. */
internal const val BACKUP_SUFFIX = ".kiteconfig.bak"
...
internal fun applyTextRewritePlan(
    ...
    val lockDirectory = safeRoot.toPath().resolve(".gradle/kiteconfig")
...
private fun stageSibling(target: Path, bytes: ByteArray): Path {
    ...
    val tmp = Files.createTempFile(target.parent, ".${target.fileName}.kiteconfig-", ".tmp")
    try {
...
private fun moveCurrentPathAside(path: Path): Path? {
    ...
    val parked = Files.createTempFile(path.parent, ".${path.fileName}.kiteconfig-current-", ".tmp")
    Files.delete(parked)
    return try {
...
internal fun renderUnifiedDiff(file: File, old: String, new: String): String {
    ...
    return buildString {
        ...
        appendLine("--- $safePath")
        appendLine("+++ $safePath (kiteConfig plan)")
...
internal fun writeBytesSafely(
    ...
    if (dryRun) {
        logger.lifecycle("[kiteConfig][dry-run] would write $label (${file.path})")
        return false
    }
