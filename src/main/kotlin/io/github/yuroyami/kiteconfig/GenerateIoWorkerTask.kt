 ...
 *
 * Writes into `build/generated/kiteconfig/<target>Main/kotlin`, a distinct
...
abstract class GenerateIoWorkerTask : DefaultTask() {
    ...
    init {
        group = "kiteconfig"
        description = "Generate the inline Web Worker (kiteConfigOffload) helper into the JS source set."
    ...
    fun generate() {
        ...
        if (dryRun.getOrElse(false)) {
            logger.info("[kiteConfig] dryRun does not suppress build-owned Web Worker generation.")
        }
        val relative = "${pkg.replace('.', '/')}/KiteConfigIoWorker.kt"
        ...
        logger.info("[kiteConfig] Generated Web Worker helper: ${root.resolve(relative)}")
    }
}
