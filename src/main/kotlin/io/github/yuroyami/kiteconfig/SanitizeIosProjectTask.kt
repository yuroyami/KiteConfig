...
abstract class SanitizeIosProjectTask : DefaultTask() {
    ...
    init {
        group = "kiteconfig"
        description = "Explicitly plan/apply KiteConfig keys and ios{} flags to a source XML Info.plist."
    ...
    fun sanitize() {
        ...
        val newText = result.text ?: run {
            logger.info("[kiteConfig] Info.plist already sanitized.")
            return
        }
        ...
        if (applied.written > 0) {
            ...
            logger.lifecycle("[kiteConfig] Info.plist sanitized: ${parts.joinToString("; ")}.")
        } else if (applied.dryRun) {
            logger.lifecycle("[kiteConfig] Info.plist plan contains one change; dry-run left the file untouched.")
        }
    }
}
