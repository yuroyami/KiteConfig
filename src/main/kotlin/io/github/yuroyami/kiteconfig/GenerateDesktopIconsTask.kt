...
abstract class GenerateDesktopIconsTask : DefaultTask() {
    ...
    fun generate() {
        if (dryRun.getOrElse(false)) {
            logger.info("[kiteConfig] dryRun does not suppress build-owned desktop icon generation.")
        }
        ...
        logger.info("[kiteConfig] Generated desktop icons: ${outputDir.get().asFile}")
    }
